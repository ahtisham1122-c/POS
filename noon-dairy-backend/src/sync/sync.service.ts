...
export class SyncService {
  ...
  async pullData(deviceId: string, since: string) {
    ...
    // Pull shared baseline data plus owner-entered supplier data. We still do
    // not pull sales/expenses generated by other terminals into the counter.
    const [products, customers, dailyRates, settings, suppliers, milkCollections, supplierPayments, supplierLedgerEntries, stockMovements] = await Promise.all([
      ...
      this.prisma.supplier.findMany({ where: { updatedAt: { gt: dateQuery } } }),
      this.prisma.milkCollection.findMany({ where: { createdAt: { gt: dateQuery } } }),
      this.prisma.supplierPayment.findMany({ where: { createdAt: { gt: dateQuery } } }),
      this.prisma.supplierLedgerEntry.findMany({ where: { createdAt: { gt: dateQuery } } }),
      this.prisma.stockMovement.findMany({
        where: {
          createdAt: { gt: dateQuery },
          movementType: 'MILK_COLLECTION'
        }
      })
    ]);
