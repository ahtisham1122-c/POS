...
function initializeDatabase() {
    ...
    const defaultAdmin = db_1.default.prepare('SELECT id, password_hash FROM users WHERE username = ?').get('admin');
    if (defaultAdmin?.password_hash === '1234') {
        db_1.default.prepare('UPDATE users SET password_hash = ?, updated_at = ? WHERE id = ?')
            .run(bcryptjs_1.default.hashSync('1234', 12), new Date().toISOString(), defaultAdmin.id);
    }
    // Seed default settings
    const settingsCount = db_1.default.prepare('SELECT COUNT(*) as count FROM settings').get().count;
    if (settingsCount === 0) {
        ...
        const insertSetting = db_1.default.prepare("INSERT INTO settings (key, value, updated_at) VALUES (?, ?, ?)");
        defaultSettings.forEach(([key, val]) => insertSetting.run(key, val, now));
    }
    const ensureSetting = db_1.default.prepare(`
  ...
  `);
    ...
    const apiUrlSetting = db_1.default.prepare("SELECT value FROM settings WHERE key = 'APP_API_URL'").get();
    if (String(apiUrlSetting?.value || '').includes('supabase.co/rest/v1')) {
        ...
        db_1.default.prepare("UPDATE settings SET value = ?, updated_at = ? WHERE key = 'SYNC_DEVICE_SECRET'")
            .run(defaultSyncSecret, nowForSettings);
    }
    // Seed default categories/products
    const productCount = db_1.default.prepare('SELECT COUNT(*) as count FROM products').get().count;
    if (productCount === 0) {
        ...
        const insertProduct = db_1.default.prepare(`
      INSERT INTO products (id, code, name, category, unit, selling_price, cost_price, stock, tax_exempt, emoji, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        seedProducts.forEach((p) => insertProduct.run(p.id, p.code, p.name, p.category, p.unit, p.selling_price, p.cost_price, p.stock, p.tax_exempt || 0, p.emoji, now, now));
    }
    // Ensure daily rates exist for today
    const today = new Date().toISOString().split('T')[0];
    const rateExists = db_1.default.prepare('SELECT COUNT(*) as count FROM daily_rates WHERE date = ?').get(today);
