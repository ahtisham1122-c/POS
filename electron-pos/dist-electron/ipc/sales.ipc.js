...
exports.registerSalesIPC = registerSalesIPC;
...
function resolveSaleUnitPrice(product, dailyRate) {
...
function calculateDiscount(subtotal, discountType, discountValueInput) {
...
function calculateItemDiscount(grossLineTotal, discountType, discountValueInput) {
    ...
    if (discountType === 'PERCENT') {
        if (discountValue > 100)
            throw new Error('Item percentage discount cannot be more than 100%');
        return { discountType, discountValue, discountAmount: roundMoney(grossLineTotal * (discountValue / 100)) };
    }
    if (discountType === 'RS') {
        if (discountValue > grossLineTotal)
...
function calculateTax({ subtotal, taxableSubtotal, discountAmount, taxEnabled, taxRate }) {
...
function registerSalesIPC() {
    electron_1.ipcMain.handle('sales:getAll', (_event, filters) => {
    ...
    electron_1.ipcMain.handle('sales:getOne', (_event, id) => {
        const sale = db_1.default.prepare('SELECT * FROM sales WHERE id = ?').get(id);
        if (!sale)
            return null;
        const items = db_1.default.prepare('SELECT * FROM sale_items WHERE sale_id = ? ORDER BY created_at ASC').all(id);
        return { ...sale, items };
    });
    electron_1.ipcMain.handle('sales:getReceipt', (_event, id) => {
        const sale = db_1.default.prepare('SELECT * FROM sales WHERE id = ?').get(id);
        if (!sale)
            return null;
        const items = db_1.default.prepare('SELECT * FROM sale_items WHERE sale_id = ? ORDER BY created_at ASC').all(id);
        const splitPayments = db_1.default.prepare('SELECT * FROM split_payments WHERE sale_id = ? ORDER BY created_at ASC').all(id);
        const customer = sale.customer_id
            ? db_1.default.prepare('SELECT id, name, phone FROM customers WHERE id = ?').get(sale.customer_id)
            : null;
        return { sale, items, customer, splitPayments };
    });
    electron_1.ipcMain.handle('sales:void', async (_event, data) => {
        try {
            return db_1.default.transaction((input) => {
                const manager = (0, auth_ipc_1.requireManagerApproval)(input.managerPin, 'voiding a sale');
        ...
        `).get(input.saleId);
                ...
                if (sale.status === 'CANCELLED')
                    throw new Error('This bill is already voided');
                ...
                const alreadyVoided = db_1.default.prepare('SELECT id FROM sale_voids WHERE sale_id = ?').get(sale.id);
                if (alreadyVoided)
                    throw new Error('This bill already has a void record');
                ...
                if (restockItems) {
                    const saleItems = db_1.default.prepare('SELECT * FROM sale_items WHERE sale_id = ?').all(sale.id);
          ...
          `);
                    for (const item of saleItems) {
                        const stockRow = getProductStock.get(item.product_id);
                        const stockBefore = Number(stockRow?.stock || 0);
                        const quantity = Number(item.quantity || 0);
                        const stockAfter = stockBefore + quantity;
                        updateProductStock.run(stockAfter, now, item.product_id);
                        (0, outboxHelper_1.createOutboxEntry)('products', 'UPDATE', item.product_id, {
                            id: item.product_id,
                            stock: stockAfter,
                            updated_at: now
                        });
                        const movementId = crypto.randomUUID();
                        insertStockMovement.run(movementId, item.product_id, quantity, stockBefore, stockAfter, voidId, `Void bill ${sale.bill_number}: ${reason}`, manager.id, now);
          ...
          `).run(ledgerId, sale.customer_id, sale.id, creditReversed, balanceAfter, `Void bill ${sale.bill_number}: ${reason}`, now, now);
        ...
        `).run(voidId, sale.id, sale.shift_id || null, sale.bill_number, manager.id, now, reason, cashReversed, creditReversed, restockItems ? 1 : 0, now);
                ...
                db_1.default.prepare('UPDATE sales SET status = ?, notes = ?, synced = 0 WHERE id = ?')
    ...
    electron_1.ipcMain.handle('sales:create', async (_event, saleData) => {
        const transaction = db_1.default.transaction((data) => {
            const now = new Date().toISOString();
            const saleId = crypto.randomUUID();
            const { terminalNumber } = (0, deviceInfo_1.getDeviceInfo)();
            const requestedCashierId = data.cashierId || (0, auth_ipc_1.getCurrentUser)()?.id;
            if (!requestedCashierId) {
                throw new Error('No logged-in cashier found');
            }
            const cashier = db_1.default.prepare('SELECT id FROM users WHERE id = ? AND is_active = 1').get(requestedCashierId);
            if (!cashier) {
      ...
      `).get();
            ...
            const existingSale = db_1.default.prepare(`
      ...
      `).get(transactionId);
            ...
            const customer = data.customerId
                ? db_1.default.prepare('SELECT id, current_balance FROM customers WHERE id = ? AND is_active = 1').get(data.customerId)
                : null;
            if (data.customerId && !customer) {
                throw new Error('Selected customer was not found');
            }
            const dailyRate = db_1.default.prepare('SELECT * FROM daily_rates ORDER BY date DESC LIMIT 1').get();
            ...
            const normalizedItems = data.items.map((item) => {
                const product = db_1.default.prepare('SELECT * FROM products WHERE id = ? AND is_active = 1').get(item.productId);
                if (!product) {
            ...
            const taxableSubtotal = roundMoney(normalizedItems.reduce((sum, item) => sum + (item.taxExempt ? 0 : item.lineTotal), 0));
            const discount = calculateDiscount(subtotal, data.discountType || 'NONE', data.discountValue);
            const totalItemDiscount = roundMoney(normalizedItems.reduce((sum, item) => sum + item.discountAmount, 0));
            ...
            if (totalDiscountGiven > discountApprovalLimit) {
                discountApprover = (0, auth_ipc_1.requireManagerApproval)(data.managerPin, `discount above Rs. ${discountApprovalLimit}`);
            }
            ...
            const cashTendered = paymentType === 'CASH'
                ? roundMoney(requestedCashTendered > 0 ? requestedCashTendered : grandTotal)
                : paymentType === 'SPLIT'
                    ? cashPaid
                    : 0;
            ...
            db_1.default.prepare('UPDATE bill_counter SET last_number = last_number + 1 WHERE id = 1').run();
            const counterResult = db_1.default.prepare('SELECT last_number FROM bill_counter WHERE id = 1').get();
      ...
      `).run(saleId, transactionId, openShift.id, billNumber, now, data.customerId || null, cashierId, paymentType, subtotal, discount.discountType, discount.discountValue, discount.discountAmount, tax.taxEnabled ? 1 : 0, taxLabel, taxRate, tax.taxableAmount, tax.taxAmount, grandTotal, amountPaid, cashTendered, changeReturned, balanceDue, 'COMPLETED', now);
            ...
            if (normalizedItems.length > 0) {
                const insertItem = db_1.default.prepare(`
        ...
        `);
                for (const item of normalizedItems) {
                    ...
                    insertItem.run(itemId, saleId, item.productId, item.productName, item.unit, item.quantity, item.sellingPrice, item.costPrice || 0, item.discountType, item.discountValue, item.discountAmount, item.lineTotal, now);
                    (0, outboxHelper_1.createOutboxEntry)('sale_items', 'INSERT', itemId, { ...item, id: itemId, sale_id: saleId, created_at: now });
                    // 3. Stock management
                    ...
                    if (currentStockObj) {
                        ...
                        (0, outboxHelper_1.createOutboxEntry)('products', 'UPDATE', item.productId, { id: item.productId, stock: currentStock - item.quantity, updated_at: now });
                        // 4. Stock Movement
                        const movId = crypto.randomUUID();
                        insertMovement.run(movId, item.productId, 'STOCK_OUT', item.quantity, currentStock, currentStock - item.quantity, saleId, cashierId, now);
            ...
            const insertSplitPayment = db_1.default.prepare(`
      ...
      `);
            if (cashPaid > 0) {
                ...
                (0, outboxHelper_1.createOutboxEntry)('split_payments', 'INSERT', splitPaymentId, {
                    id: splitPaymentId,
                    sale_id: saleId,
                    method: 'CASH',
                    amount: cashPaid,
            ...
            if (onlinePaid > 0) {
                ...
                (0, outboxHelper_1.createOutboxEntry)('split_payments', 'INSERT', onlineSplitPaymentId, {
            ...
            if (data.customerId && paymentType === 'CREDIT') {
                ...
                (0, outboxHelper_1.createOutboxEntry)('split_payments', 'INSERT', creditSplitPaymentId, {
                ...
                db_1.default.prepare(`UPDATE customers SET current_balance = ?, updated_at = ? WHERE id = ?`).run(newBalance, now, data.customerId);
                (0, outboxHelper_1.createOutboxEntry)('customers', 'UPDATE', data.customerId, { id: data.customerId, current_balance: newBalance, updated_at: now });
                const ledgerId = crypto.randomUUID();
                db_1.default.prepare(`
    ...
    electron_1.ipcMain.handle('sales:hold', async (_event, holdData) => {
        const transaction = db_1.default.transaction((data) => {
            const now = new Date().toISOString();
      ...
      `);
            for (const item of data.items) {
                insertItem.run(crypto.randomUUID(), holdId, item.productId, item.productName, item.unit, item.quantity, item.price, item.lineTotal, now);
            }
    ...
    electron_1.ipcMain.handle('sales:getHeld', () => {
        try {
            const heldSales = db_1.default.prepare('SELECT * FROM held_sales ORDER BY created_at ASC').all();
            return heldSales.map(sale => {
                const items = db_1.default.prepare('SELECT * FROM held_sale_items WHERE held_sale_id = ?').all(sale.id);
                return {
                    id: sale.id,
    ...
    electron_1.ipcMain.handle('sales:deleteHeld', (_event, id) => {
        const transaction = db_1.default.transaction((holdId) => {
            db_1.default.prepare('DELETE FROM held_sale_items WHERE held_sale_id = ?').run(holdId);
            db_1.default.prepare('DELETE FROM held_sales WHERE id = ?').run(holdId);
            return { success: true };
        });
        try {
