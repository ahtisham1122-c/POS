...
function resolveSaleUnitPrice(product: any, dailyRate: any) {
...
function calculateDiscount(subtotal: number, discountType: string, discountValueInput: unknown) {
...
function calculateItemDiscount(grossLineTotal: number, discountType: string, discountValueInput: unknown) {
...
export function registerSalesIPC() {
  ...
  ipcMain.handle('sales:void', async (_event, data: { saleId: string; reason: string; restockItems?: boolean; managerPin?: string }) => {
    try {
      return db.transaction((input) => {
        ...
        if (cashReversed > 0) {
          addCashOut(cashReversed, now.split('T')[0]);
        }
  ...
  ipcMain.handle('sales:create', async (_event, saleData) => {
    const transaction = db.transaction((data) => {
      ...
      const saleDate = getBusinessDate(new Date(now));
      ...
      const cashRegister = db.prepare('SELECT * FROM cash_register WHERE date = ?').get(saleDate) as any;
      if (!cashRegister) {
      ...
      `).run(
        saleId, transactionId, billNumber, now, data.customerId || null, cashierId, paymentType,
      ...
      if (cashPaid > 0) {
        addCashIn(Number(cashPaid), saleDate);
      }
