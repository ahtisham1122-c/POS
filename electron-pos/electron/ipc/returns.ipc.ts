...
type CreateReturnInput = {
  ...
  correctionType?: 'CORRECTION' | 'REFUND';
};
      ...
      return db.transaction((input: CreateReturnInput) => {
        ...
        db.prepare(`
          INSERT INTO returns (
            ...
            correction_type, status, created_at, synced
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'COMPLETED', ?, 0)
        `).run(
          returnId,
          returnNumber,
        ...
        logAudit({
          actionType: correctionType === 'CORRECTION' ? 'CORRECTION_CREATED' : 'REFUND_CREATED',
          entityType: 'returns',
          entityId: returnId,
          before: { saleId: sale.id, billNumber: sale.bill_number },
          after: { returnNumber, refundMethod: input.refundMethod, refundAmount, restockItems, correctionType },
          reason: input.reason.trim(),
          approvedBy: approver
        });
        ...
        } else if (Number(returnedTotal?.total || 0) >= Number(sale.grand_total || 0)) {
          newStatus = 'REFUNDED';
        }
